...
public class TaskServiceImplTest {
    ...
    @Test
    public void changeTask_shouldThrowsExc(){
        TaskUpdate taskUpdate = new TaskUpdate(TEST_UPDATE_TITLE, TEST_UPDATE_DESC);

        when(repository.findById(TASK_ID)).thenReturn(Optional.empty());
        assertThrows(TaskNotFoundException.class, ()->taskService.changeTask(TASK_ID,taskUpdate));
    }

    @Test
    public void changeTask_FieldNull() {
        TaskUpdate taskUpdate = new TaskUpdate(null, null);
        when(repository.findById(TASK_ID)).thenReturn(Optional.of(task));
        taskService.changeTask(TASK_ID, taskUpdate);
        verify(repository).save(task);
